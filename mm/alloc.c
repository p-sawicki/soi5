...
unsigned int worstFitOn;

PUBLIC int do_hole_map(void){
	unsigned int i;
	struct hole *hp;
	size_t nbytes, length;
	unsigned int *buffer = (unsigned int *)mm_in.m1_p1;
	unsigned int bufferBeg[NR_HOLES * 2 + 1];
	nbytes = mm_in.m1_i1;
	hp = hole_head;
	length = (nbytes - 4) / 8;
	if(hp == NIL_HOLE)
		return 0;
	for(i = 0; i < length;){
		bufferBeg[2 * i] = hp->h_len;
		bufferBeg[2 * i + 1] = hp->h_base;
		++i;
	...
	bufferBeg[2 * i] = 0;
	sys_copy(MM_PROC_NR, D, (phys_bytes) bufferBeg, who, D, (phys_bytes) buffer, (phys_bytes) (i * 8 + 4));
	return i;
}
PUBLIC int do_worst_fit(void){
	unsigned int w = mm_in.m1_i1;
	if(w == 1)
		worstFitOn = 1;
	else if(w == 0)
		worstFitOn = 0;
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *worst_ptr;
  ...
  do {
	...
	while (hp != NIL_HOLE && hp->h_base < swap_base) {
 	...
 	if(worst_ptr)
 		...
 		return(NO_MEM);	
 	while(hp != NIL_HOLE){
 		if(hp->h_len > worst_ptr->h_len){
 			prev_ptr = worst_ptr;
 			worst_ptr = hp;
 		}
 		hp = hp->h_next;
 	}
	old_base = worst_ptr->h_base;	/* remember where it started */
	worst_ptr->h_base += clicks;	/* bite a piece off */
	worst_ptr->h_len -= clicks;	/* ditto */
	if (worst_ptr->h_len == 0) del_slot(prev_ptr, worst_ptr);
	return(old_base);
 }
  return(NO_MEM);
}
